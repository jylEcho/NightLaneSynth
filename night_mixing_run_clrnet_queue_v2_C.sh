...
CLRNET_ROOT=/root/autodl-tmp/CLRNet_laneC_v2
LOG_ROOT="$STUDY_ROOT/logs"
mkdir -p "$LOG_ROOT"
cd "$CLRNET_ROOT"

for exp_name in "$@"; do
  echo "===== START ${exp_name} $(date -Iseconds) lane=C =====" | tee -a "$LOG_ROOT/queue_C.log"
  rm -rf cache
  ...
  echo "===== DONE ${exp_name} $(date -Iseconds) lane=C =====" | tee -a "$LOG_ROOT/queue_C.log"
done
