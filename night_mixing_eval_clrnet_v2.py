...
def main() -> None:
    ...
    res_real = eval_predictions(str(pred_dir), str(data_root), str(real_test), iou_thresholds=np.linspace(0.5, 0.95, 10), official=True)
    res_syn = eval_predictions(str(pred_dir), str(data_root), str(syn_test), iou_thresholds=np.linspace(0.5, 0.95, 10), official=True)
